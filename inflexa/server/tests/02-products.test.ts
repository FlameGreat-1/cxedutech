...
describe('Product Flow - Admin CRUD, Public Browse, Filters', () => {
  it('should create a product as admin', async () => {
    const res = await request
      ...
      .send({
        title: 'Phonics Flashcards Ages 3-5', description: 'Learn letter sounds with fun illustrations',
        min_age: 3, max_age: 5, subject: 'English', focus_area: 'Phonics',
        price: 14.99, currency: 'GBP', format: 'physical',
        included_items: ['40 flashcards', 'Parent guide'], inventory_count: 100,
      })
      .expect(201);
  ...
  it('should create a second product for filtering', async () => {
    const res = await request
      ...
      .set('Authorization', `Bearer ${adminToken}`)
      .send({
        title: 'Maths Flashcards Ages 5-7', description: 'Addition and subtraction practice',
        min_age: 5, max_age: 7, subject: 'Maths', focus_area: 'Arithmetic',
        price: 11.99, format: 'printable', included_items: ['50 printable cards'], inventory_count: 200,
      })
      .expect(201);
  ...
  it('should reject product creation without token', async () => {
    await request.post('/api/admin/products').send({ title: 'Hack' }).expect(401);
  });

  it('should reject product creation with user token', async () => {
    await request.post('/api/admin/products').set('Authorization', `Bearer ${userToken}`)
      .send({ title: 'Hack', description: 'x', min_age: 1, max_age: 2, subject: 'x', focus_area: 'x', price: 1, format: 'physical', included_items: ['x'] })
      .expect(403);
  });

  it('should reject product with missing required fields', async () => {
    const res = await request.post('/api/admin/products').set('Authorization', `Bearer ${adminToken}`).send({ title: 'Incomplete' }).expect(400);
  ...
  it('should update a product as admin', async () => {
    const res = await request.put(`/api/admin/products/${productId}`).set('Authorization', `Bearer ${adminToken}`)
      .send({ price: 16.99, title: 'Phonics Flashcards Ages 3-5 (Updated)' }).expect(200);
  ...
  it('should update inventory independently', async () => {
    const res = await request.patch(`/api/admin/products/${productId}/inventory`).set('Authorization', `Bearer ${adminToken}`)
      .send({ inventory_count: 75 }).expect(200);
    expect(res.body.data.inventory_count).toBe(75);
  });

  it('should delete a product as admin', async () => {
    const createRes = await request.post('/api/admin/products').set('Authorization', `Bearer ${adminToken}`)
      .send({ title: 'To Delete', description: 'Will be deleted', min_age: 1, max_age: 3, subject: 'Test', focus_area: 'Test', price: 5, format: 'physical', included_items: ['test'], inventory_count: 1 })
      .expect(201);
    await request.delete(`/api/admin/products/${createRes.body.data.id}`).set('Authorization', `Bearer ${adminToken}`).expect(200);
    await request.get(`/api/products/${createRes.body.data.id}`).expect(404);
  });
});
