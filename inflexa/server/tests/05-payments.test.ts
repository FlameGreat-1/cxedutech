import { request, cleanAllTables, createAdminUser, registerUser, createProduct, createOrder, closeDb, db } from './helpers';
...
beforeAll(async () => {
  ...
  const product = await createProduct(adminToken, { title: 'Payment Test Cards', price: 25.00, inventory_count: 100 });
  const order = await createOrder(userToken, product.id as number, 1);
  orderId = order.id as number;
  const guestRes = await request.post('/api/orders/guest')
    .send({ items: [{ product_id: product.id, quantity: 1 }], shipping: { shipping_name: 'Guest Payer', shipping_email: 'guestpay@test.com', shipping_address_line1: '1 Guest Lane', shipping_city: 'London', shipping_state: 'England', shipping_postal_code: 'E1 1AA' } })
    .expect(201);
...
describe('Payment Flow - Intent Creation, Validation, Records', () => {
  ...
  it('should create a payment intent for a pending order', async () => {
    const res = await request.post('/api/payments/create-intent').set('Authorization', `Bearer ${userToken}`).send({ order_id: orderId }).expect(201);
  ...
  it('should return same intent on duplicate request', async () => {
    const res = await request.post('/api/payments/create-intent').set('Authorization', `Bearer ${userToken}`).send({ order_id: orderId }).expect(201);
    expect(res.body.data.payment.id).toBe(paymentId);
  });

  it('should reject payment intent for non-existent order', async () => {
    await request.post('/api/payments/create-intent').set('Authorization', `Bearer ${userToken}`).send({ order_id: 99999 }).expect(404);
  });
  ...
  it('should reject payment intent when user does not own the order', async () => {
    ...
    await request.post('/api/payments/create-intent').set('Authorization', `Bearer ${other.token}`).send({ order_id: orderId }).expect(403);
  });

  it('should create a guest payment intent', async () => {
    const res = await request.post('/api/payments/guest/create-intent').send({ order_id: guestOrderId }).expect(201);
    expect(res.body.data.clientSecret).toBeDefined();
    expect(res.body.data.payment.order_id).toBe(guestOrderId);
  });

  it('should get payment details by ID', async () => {
    const res = await request.get(`/api/payments/${paymentId}`).set('Authorization', `Bearer ${userToken}`).expect(200);
  ...
  it('should reject payment intent without order_id', async () => {
    await request.post('/api/payments/create-intent').set('Authorization', `Bearer ${userToken}`).send({}).expect(400);
  });
